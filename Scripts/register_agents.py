    ...
    for a in agents:
        ...
        if validate_imports:
            ...
            try:
                ...
                build = next(
                    (getattr(gmod, fn) for fn in dir(gmod)
                     if fn.startswith("build_") and fn.endswith("_graph") and callable(getattr(gmod, fn))),
                    None,
                )
                if build:
                    graph = build()
...
def print_table(agents: list[dict]) -> None:
    ...
    for a in agents:
        ...
        imp   = ("✓" if a["import_ok"] is True else ("✗" if a["import_ok"] is False else "–"))
        grph  = ("✓" if a["graph_ok"]  is True else ("✗" if a["graph_ok"]  is False else "–"))
        hitl  = "HITL" if a["hitl_enabled"] else "    "
        print(f"  [{enabled}] {imp}agent {grph}graph  {hitl}  {a['name']:<30}  actions: {', '.join(a['actions'])}")
...
def main() -> None:
    ...
    parser.add_argument("--validate", action="store_true", help="Validate agent imports and graphs")
