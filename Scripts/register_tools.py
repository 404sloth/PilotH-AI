...
def print_table(tools: list[dict]) -> None:
    ...
    for t in tools:
        import_flag = (
            "  ✓"
            if t["import_ok"] is True
            else "  ✗"
            if t["import_ok"] is False
            else "  –"
        )
        print(
            f"  {t['name']:<30} {t['owner_agent']:<25} {'yes':>4} "
            if t["requires_llm"]
            else f"  {t['name']:<30} {t['owner_agent']:<25}  no  {'yes':>5}"
            if t["requires_credentials"]
            else f" no  {import_flag}"
        )
    ...
    for t in tools:
        ...
        imp = (
            "✓" if t["import_ok"] is True else ("✗" if t["import_ok"] is False else "–")
        )
