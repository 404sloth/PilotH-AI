...
try:
    ...
    check(
        "find_persons — returns seeded persons",
        len(all_persons) >= 5,
        f"{len(all_persons)} found",
    )

    p = get_person_by_email("anil.yadav@company.com")
    check(
        "get_person_by_email anil.yadav",
        p is not None and p["department"] == "Engineering",
    )

    p2 = get_person_by_email("anil.kumar@company.com")
    check(
        "get_person_by_email anil.kumar (Finance — different Anil)",
        p2 is not None and p2["department"] == "Finance",
    )

    # Disambiguation: same first name but different people
    anil_results = find_persons(name="Anil")
    check(
        "Disambiguation: 2 Anils found",
        len(anil_results) == 2,
    ...
    check(
        "get_meeting_full MTG-001",
        mtg is not None and len(mtg.get("attendees", [])) >= 2,
    )
...
try:
    ...
    out = t.execute(
        BriefingInput(emails=["anil.yadav@company.com", "james.carter@company.com"])
    )
    check("ParticipantBriefingTool — found 2", out.found == 2)
    check(
        "ParticipantBriefingTool — disambiguation note present",
        bool(out.participants[0].disambiguation_note),
    )
except Exception as e:
...
try:
    ...
    check(
        "GoogleCalendarAvailabilityTool — checked attendees",
        len(avail.checked) >= 1,
        f"checked={avail.checked}",
    )

    created = GoogleCalendarCreateTool().execute(
        CalendarCreateInput(
            title="Test Meeting",
    ...
    check(
        "GoogleCalendarCreateTool — internal only (no HITL)", not created.has_external
    )
except Exception as e:
    check("Calendar Tools", False, str(e))

try:
    from agents.communication.tools.timezone_tool import (
        TimezoneConverterTool,
        TimezoneInput,
    )

    out = TimezoneConverterTool().execute(
        TimezoneInput(
...
try:
    ...
    out = SentimentAnalysisTool().execute(
        SentimentInput(
            texts=[
                "The project is going great and the team is impressed!",
                "There are serious concerns about delays.",
            ]
        )
    )
    check("SentimentAnalysisTool — analysed 2 texts", len(out.records) == 2)
    check(
        "SentimentAnalysisTool — first text positive",
        out.records[0].label == "positive",
    )
    check(
        "SentimentAnalysisTool — second text negative",
        out.records[1].label in ("negative", "neutral"),
    )
except Exception as e:
...
try:
    ...
    out = AgendaGeneratorTool().execute(
        AgendaInput(
    ...
    check(
        "AgendaGeneratorTool — has items",
        len(out.items) >= 3,
        f"{len(out.items)} items",
    )
    check(
        "AgendaGeneratorTool — total duration = 60",
        out.total_minutes == 60,
        f"total={out.total_minutes}",
    )
except Exception as e:
...
try:
    ...
    """
    out = MeetingSummarizerTool().execute(
        SummarizerInput(
    ...
    check(
        "MeetingSummarizerTool — extracted decisions or action items",
        len(out.key_decisions) + len(out.action_items) > 0,
    )
except Exception as e:
...
try:
    ...
    check(
        "ConflictResolverTool — returns result", "conflict_detected" in out.model_dump()
    )
    check(
        "ConflictResolverTool — alternatives list present",
        isinstance(out.alternatives, list),
    )
except Exception as e:
...
try:
    ...
    check(
        "resolve_participants_node — found 2",
        len(s1.get("resolved_participants", [])) == 2,
    )
    ...
    check(
        "find_common_slots_node — free slots returned",
        len(s3.get("free_slots", [])) > 0,
    )

    state.update(s3)
    s4 = propose_slots_node(state)
    check(
        "propose_slots_node — proposed slots list",
        len(s4.get("proposed_slots", [])) > 0,
    )
...
try:
    ...
    check(
        "extract_key_points_node — returns dicts",
        "decisions" in kp or "key_points" in kp or kp == {},
    )
...
try:
    from agents.communication.nodes.briefing import (
        gather_context_node,
    ...
    check(
        "generate_agenda_node — agenda items list", len(ag.get("agenda_items", [])) >= 1
    )
...
try:
    ...
    check(
        "SessionStore — context set",
        s.get_context("last_agent") == "meetings_communication",
    )
