...
try:
    from integrations.data_warehouse.vendor_db import (
        search_vendors, get_vendor_by_id, find_best_vendors_for_service,
        get_contract_details, get_sla_compliance, get_milestones,
        get_client_project, save_vendor_selection, get_saved_selections,
    ...
    check("get_vendor_by_id V-001", v is not None and v["name"] == "Acme Cloud Solutions",
          v["name"] if v else "None")

    best = find_best_vendors_for_service("cloud_hosting", {"min_quality_score": 80, "min_on_time_rate": 0.88})
    check("find_best_vendors_for_service", len(best) >= 1, f"{len(best)} candidates, top={best[0].get('name') if best else 'N/A'}")
    check("Ranked by fit_score descending", all(
        best[i]["fit_score"] >= best[i+1]["fit_score"] for i in range(len(best)-1)
    ))

    contract = get_contract_details(vendor_id="V-001")
    check("get_contract_details V-001", contract is not None and contract.get("contract_reference") == "CTR-2024-001")

    sla = get_sla_compliance("V-001")
    check("get_sla_compliance V-001", sla is not None and "metrics" in sla, f"compliance={sla.get('overall_compliance') if sla else 'N/A'}%")
    ...
    check("save & retrieve vendor_selection", any(s["vendor_id"] == "V-001" for s in sels))
...
try:
    ...
    check("VendorSearchTool — found vendors", out.found and out.count >= 2, f"count={out.count}")
...
try:
    ...
    t = VendorMatcherTool()
    out = t.execute(VendorMatcherInput(
    ...
    check("VendorMatcherTool — candidates found", out.candidates_found >= 1, f"top={out.top_recommendation}")
    check("VendorMatcherTool — ranked by fit_score", out.ranked_vendors[0].rank == 1 if out.ranked_vendors else True)
    check("VendorMatcherTool — top_recommendation set", out.top_recommendation is not None)
except Exception as e:
    check("VendorMatcherTool", False, str(e))

try:
    from agents.vendor_management.tools.contract_parser import ContractParserTool, ContractParserInput
    ...
    check("ContractParserTool — has deliverables", out.found and len(out.contract.deliverables) > 0)
...
try:
    ...
    check("SLAMonitorTool — has metrics", len(out.metrics) > 0, f"{len(out.metrics)} metrics")
    check("SLAMonitorTool — compliance 0-100", 0 <= out.overall_compliance <= 100, f"{out.overall_compliance}%")
except Exception as e:
...
try:
    ...
    check("MilestoneTrackerTool — overall_status set", out.overall_status in ("on_track", "at_risk", "delayed"))
except Exception as e:
...
try:
    ...
    check("VendorScorecardTool — score in range", 0 <= out.overall_score <= 100, f"score={out.overall_score}")
...
try:
    ...
    check("fetch_vendor_node FIND_BEST — returns ranked_vendors", "ranked_vendors" in result and len(result["ranked_vendors"]) >= 1)
    check("fetch_vendor_node FIND_BEST — top_recommendation set", result.get("top_recommendation") is not None)

    # Test single vendor fetch
    state2 = {"action": "full_assessment", "vendor_name": "Acme", "vendor_id": None}
    result2 = fetch_vendor_node(state2)
    check("fetch_vendor_node ASSESS — vendor_details returned", "vendor_details" in result2 or result2.get("error"))

    # Test evaluate node (rule-based fallback)
    eval_state = {
        "action": "full_assessment",
        "vendor_details": {
            "name": "Acme Cloud Solutions", "quality_score": 91, "on_time_rate": 0.96,
            "communication_score": 88, "cost_competitiveness": 74, "innovation_score": 82,
            "avg_client_rating": 4.7, "total_projects_completed": 142,
        },
        "vendor_id": "V-001",
        "sla_data": {"overall_compliance": 75.0, "breaches": ["Resolution Time exceeded"]},
        "milestone_data": [],
    }
    eval_result = evaluate_node(eval_state)
    check("evaluate_node — scores returned", "evaluation_scores" in eval_result or eval_result == {})

    # Test risk_detect
    risk_state = {
        "action": "full_assessment",
        "evaluation_scores": {"quality": 91, "reliability": 70, "sla_compliance": 75, "communication": 88, "cost": 74, "innovation": 82},
        "sla_data": {"overall_compliance": 75.0, "breaches": ["Resolution Time exceeded"]},
        "milestone_data": [{"status": "delayed", "name": "Dev Phase", "days_overdue": 12}],
    ...
    check("risk_detect_node — identifies delayed milestone", any(
        r.get("category") == "project" for r in risk_result.get("risk_items", [])
    ))
...
try:
    from agents.vendor_management.schemas import VendorManagementInput, VendorManagementOutput
    ...
    out = VendorManagementOutput(action_performed="find_best", ranked_vendors=[], recommendations=["Test"])
