...
def seed_meeting_data() -> None:
    ...
    with get_db_connection() as conn:
        ...
        persons = [
            ...
            ("P-002", "Priya Sharma",      "priya.sharma@company.com",    "Product",        "PilotH",    "Product Manager",   "Mumbai",    "Asia/Kolkata",  "P-001", "+91-98765-00002", "@priya",   "PM driving PilotH roadmap and stakeholder management."),
            ("P-003", "James Carter",      "james.carter@company.com",    "Engineering",    "PilotH",    "Senior Engineer",   "New York",  "America/New_York", "P-001", "+1-212-555-0003", "@james", "Backend engineer specialising in LangGraph orchestration."),
            ("P-004", "Mei Lin",           "mei.lin@company.com",         "Data Science",   "Analytics", "Data Scientist",    "Singapore", "Asia/Singapore","P-001", "+65-9000-0004",  "@mei",     "Data scientist working on predictive analytics models."),
            ("P-005", "Anil Kumar",        "anil.kumar@company.com",      "Finance",        "Budget",    "Financial Analyst", "Delhi",     "Asia/Kolkata",  None,    "+91-98765-00005", "@anilk",  "Financial analyst — different person from Anil Yadav CTO."),
            ("P-006", "Sofia Martinez",    "sofia.martinez@company.com",  "Marketing",      "Growth",    "Marketing Lead",    "London",    "Europe/London", None,    "+44-20-0006",    "@sofia",   "Marketing lead overseeing growth campaigns."),
            ("P-007", "David Kim",         "david.kim@company.com",       "Engineering",    "PilotH",    "DevOps Engineer",   "Seoul",     "Asia/Seoul",    "P-001", "+82-10-0007",    "@david",   "DevOps engineer managing CI/CD and infrastructure."),
            ("P-008", "Rachel Green",      "rachel.green@company.com",    "Legal",          "Compliance","Legal Counsel",     "New York",  "America/New_York","P-001","+1-212-555-0008","@rachel", "Legal counsel overseeing compliance and contracts."),
        ]
        cur.executemany(
        ...
        skills = [
            ("P-001", "Python"), ("P-001", "LLM Engineering"), ("P-001", "System Architecture"),
            ("P-002", "Product Strategy"), ("P-002", "Agile"), ("P-002", "Stakeholder Management"),
            ("P-003", "Python"), ("P-003", "LangGraph"), ("P-003", "FastAPI"),
            ("P-004", "ML"), ("P-004", "Python"), ("P-004", "Tableau"),
            ("P-005", "Financial Modelling"), ("P-005", "Excel"), ("P-005", "SQL"),
            ("P-006", "SEO"), ("P-006", "Content Marketing"), ("P-006", "Analytics"),
            ("P-007", "Kubernetes"), ("P-007", "Terraform"), ("P-007", "GitHub Actions"),
            ("P-008", "Contract Law"), ("P-008", "GDPR"), ("P-008", "Risk Management"),
        ]
        ...
        now = datetime.utcnow()
        events = [
            ("P-001", None, "Team Standup",    (now + timedelta(hours=2)).isoformat(), (now + timedelta(hours=2, minutes=30)).isoformat(), "Asia/Kolkata",   1),
            ("P-001", None, "Board Review",    (now + timedelta(hours=5)).isoformat(), (now + timedelta(hours=7)).isoformat(),             "Asia/Kolkata",   1),
            ("P-002", None, "Sprint Planning", (now + timedelta(hours=1)).isoformat(), (now + timedelta(hours=3)).isoformat(),             "Asia/Kolkata",   1),
            ("P-003", None, "Client Call",     (now + timedelta(hours=3)).isoformat(), (now + timedelta(hours=4)).isoformat(),             "America/New_York",1),
            ("P-004", None, "Data Review",     (now + timedelta(hours=6)).isoformat(), (now + timedelta(hours=7)).isoformat(),             "Asia/Singapore", 1),
        ]
        cur.executemany(
        ...
        attendees = [("MTG-001","P-001","organizer","accepted"), ("MTG-001","P-002","attendee","accepted"),
                     ("MTG-001","P-003","presenter","accepted"), ("MTG-001","P-007","attendee","accepted")]
        ...
        cur.executemany("INSERT OR IGNORE INTO meeting_agendas(meeting_id,topic,duration_mins,presenter_id,agenda_order) VALUES(?,?,?,?,?)", agenda)

        action_items = [
            ("MTG-001", "P-003", "Implement parallel graph execution",    (now + timedelta(days=5)).date().isoformat(),  "high"),
            ("MTG-001", "P-007", "Setup monitoring stack with Grafana",   (now + timedelta(days=7)).date().isoformat(),  "medium"),
            ("MTG-001", "P-001", "Finalise architecture decision record", (now + timedelta(days=3)).date().isoformat(),  "high"),
        ]
        cur.executemany("INSERT OR IGNORE INTO meeting_action_items(meeting_id,assignee_id,description,due_date,priority) VALUES(?,?,?,?,?)", action_items)
...
def find_persons(
    ...
    with get_db_connection() as conn:
        ...
        params: List[Any] = []
        if email:
            sql += " AND p.email = ?"; params.append(email)
        if name:
            sql += " AND LOWER(p.full_name) LIKE ?"; params.append(f"%{name.lower()}%")
        if department:
            sql += " AND LOWER(p.department) LIKE ?"; params.append(f"%{department.lower()}%")
        if project:
            sql += " AND LOWER(p.project) LIKE ?"; params.append(f"%{project.lower()}%")
        if location:
            sql += " AND LOWER(p.location) LIKE ?"; params.append(f"%{location.lower()}%")
        ...
        for row in rows:
            r = dict(row)
            cur2 = conn.cursor()
            cur2.execute("SELECT skill FROM person_skills WHERE person_id=?", (r["id"],))
...
def create_calendar_event(
    ...
    with get_db_connection() as conn:
        ...
        cur.execute("""
            ...
            VALUES(?,?,?,?,?,?,1,?,?)
        """, (person_id, external_id, title, start_time, end_time, timezone, description, location))
        conn.commit()
...
def create_meeting(
    ...
    with get_db_connection() as conn:
        conn.execute("""
            ...
            VALUES(?,?,?,?,?,?,?,?,?)
        """, (meeting_id, title, organizer_id, start_time, end_time, timezone, duration_mins, description, meeting_type))
        conn.commit()
    return meeting_id
...
def add_attendees(meeting_id: str, attendees: List[Dict[str, str]]) -> None:
    ...
    with get_db_connection() as conn:
        conn.executemany(
            ...
            [(meeting_id, a["person_id"], a.get("role","attendee"), a.get("rsvp","pending")) for a in attendees]
        )
        conn.commit()
...
def save_agenda(meeting_id: str, agenda_items: List[Dict[str, Any]]) -> None:
    ...
    with get_db_connection() as conn:
        conn.executemany(
            ...
            [(meeting_id, a["topic"], a.get("duration_mins",10), a.get("presenter_id"), a.get("order",0), a.get("notes")) for a in agenda_items]
        )
        conn.commit()
...
def save_action_items(meeting_id: str, items: List[Dict[str, Any]]) -> None:
    ...
    with get_db_connection() as conn:
        conn.executemany(
            ...
            [(meeting_id, i.get("assignee_id"), i["description"], i.get("due_date"), i.get("priority","medium")) for i in items]
        )
        conn.commit()
...
def log_communication(
    ...
    with get_db_connection() as conn:
        conn.execute("""
            ...
            VALUES(?,?,?,?,?,?,?,?)
        """, (sender_id, recipient_id, channel, subject, content, sentiment_score, sentiment_label, meeting_id))
        conn.commit()
