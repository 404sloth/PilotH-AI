...
class Settings(BaseSettings):
    ...
    # ── Database ────────────────────────────────────────────
    sqlite_db_path: str = Field("pilot_db.sqlite", description="SQLite database file path")

    # ── Human-in-the-Loop ───────────────────────────────────
    hitl_threshold: float = Field(0.7, description="Risk score threshold triggering HITL")

    # ── Observability ───────────────────────────────────────
    langsmith_api_key:    str  = Field("", description="LangSmith tracing API key")
    langsmith_project:    str  = Field("piloth-vendor", description="LangSmith project name")
