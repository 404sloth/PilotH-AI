...
def search_vendors(
    ...
    service_tag: Optional[str] = Query(None, description="Filter by service capability"),
    ...
    results = dal_search(vendor_name=name, service_tag=service_tag, country=country, limit=limit)
...
def get_contract(vendor_id: str):
    ...
    if not contract:
        raise HTTPException(status_code=404, detail=f"No contract found for vendor '{vendor_id}'.")
    return contract
...
def get_sla(vendor_id: str, period_days: int = Query(30, ge=1, le=365)):
    ...
    if not sla:
        raise HTTPException(status_code=404, detail=f"No SLA data found for vendor '{vendor_id}'.")
    return sla
...
def get_milestones(vendor_id: str, project_id: Optional[str] = Query(None)):
    ...
    return {"vendor_id": vendor_id, "milestones": dal_ms(vendor_id=vendor_id, project_id=project_id)}
...
def find_best_vendors(body: FindBestVendorRequest):
    ...
    if not agent:
        raise HTTPException(status_code=503, detail="Vendor Management Agent not initialised.")
