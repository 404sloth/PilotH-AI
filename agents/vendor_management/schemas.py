...
class VendorManagementInput(BaseModel):
    ...
    action: VendorAction = Field(
        default=VendorAction.FULL_ASSESSMENT,
        description="What the agent should do"
    )

    # --- Existing vendor context (EVALUATE, MONITOR_SLA, TRACK_MILESTONES, FULL_ASSESSMENT)
    vendor_name: Optional[str] = Field(None, description="Name (or partial) of the vendor")
    vendor_id:   Optional[str] = Field(None, description="Internal vendor ID (e.g. V-001)")

    # --- New project / service matching (FIND_BEST)
    service_required:  Optional[str]  = Field(None, description="Service tag needed (e.g. cloud_hosting)")
    budget_monthly:    Optional[float] = Field(None, description="Max monthly budget in USD")
    min_quality_score: float           = Field(75.0, ge=0, le=100)
    min_on_time_rate:  float           = Field(0.85, ge=0, le=1.0)
    required_tier:     Optional[str]  = Field(None, description="preferred | standard | trial")
    country:           Optional[str]  = Field(None, description="Country ISO code (e.g. US)")
    client_project_id: Optional[str]  = Field(None, description="Client project ID for persisting result")
