...
def resolve_participants_node(state: MeetingState) -> Dict[str, Any]:
    ...
    emails = [
        p.get("email") or p if isinstance(p, str) else p.get("email", "")
...
def fetch_availability_node(state: MeetingState) -> Dict[str, Any]:
    ...
    emails = [
        p["email"] for p in (state.get("resolved_participants") or []) if p.get("email")
    ]
    if not emails:
        return {
            "availability": {},
            "messages": [
                ToolMessage(content="No emails to check.", tool_call_id="fetch_avail")
            ],
        }
    ...
    result = tool.execute(
        AvailabilityInput(attendee_emails=emails, from_time=from_t, to_time=to_t)
    )

    availability = {}
    for block in result.busy_blocks:
        availability.setdefault(block.email, []).append(
    ...
    return {
        ...
        "messages": [
            ToolMessage(content=result.summary, tool_call_id="fetch_availability")
        ],
    }
...
def propose_slots_node(state: MeetingState) -> Dict[str, Any]:
    ...
    for slot in free_slots[:3]:
        try:
            ...
            proposed.append(
                f"{dt.strftime('%A %d %b %Y %H:%M')} – {end.strftime('%H:%M')} {timezone}"
            )
        except Exception:
