...
def gather_context_node(state: MeetingState) -> Dict[str, Any]:
    ...
    if emails:
        ...
        result = tool.execute(BriefingInput(emails=emails, meeting_context=state.get("context")))
...
def analyze_sentiment_node(state: MeetingState) -> Dict[str, Any]:
    ...
    if not texts:
        return {"sentiment_results": [], "messages": [AIMessage(content="No text to analyse.")]}
...
def generate_agenda_node(state: MeetingState) -> Dict[str, Any]:
    ...
    result = tool.execute(AgendaInput(
...
def compile_briefing_node(state: MeetingState) -> Dict[str, Any]:
    ...
    avg_sentiment = (
        sum(r.get("score", 0) for r in sentiment) / len(sentiment)
        if sentiment else 0.0
    )
    ...
    return {
        ...
        "messages": [AIMessage(content=f"Briefing compiled for '{title}' ({len(bios)} attendees).")],
    }
