...
class MeetingSummarizerTool(StructuredTool):
    ...
    description: str = "Summarise a meeting transcript into key decisions, action items, and risks."
...
{{
  ...
  "sentiment": "positive|neutral|concerns_raised"
    ...
    def _rule_based(self, inp: SummarizerInput) -> SummarizerOutput:
        ...
        decisions = [l.strip() for l in lines if any(kw in l.lower() for kw in ["decided", "agreed", "approved", "confirmed"])]
        actions   = [l.strip() for l in lines if any(kw in l.lower() for kw in ["action:", "todo:", "will", "should", "must", "owner:"])]
        risks     = [l.strip() for l in lines if any(kw in l.lower() for kw in ["risk", "concern", "issue", "blocker"])]
        return SummarizerOutput(
