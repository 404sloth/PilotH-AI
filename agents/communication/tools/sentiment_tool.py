...
class SentimentInput(BaseModel):
    ...
    source_type: str        = Field("communication", description="communication|meeting_notes|email")
...
_POSITIVE = ["excellent", "great", "happy", "pleased", "impressed", "agree", "perfect", "thank", "appreciate", "well"]
_NEGATIVE = ["concern", "issue", "problem", "delay", "miss", "fail", "unhappy", "disappoint", "risk", "urgent", "blocked"]
...
class SentimentAnalysisTool(StructuredTool):
    ...
    description: str = "Analyse sentiment (positive/negative/neutral) and key phrases from text."
    ...
    def execute(self, inp: SentimentInput) -> SentimentOutput:
        ...
        return SentimentOutput(overall_score=round(avg, 3), overall_label=label, records=records, summary=summary)
    ...
    def _analyse_single(self, text: str) -> SentimentRecord:
        ...
        return SentimentRecord(text=text[:200], score=score, label=label, key_phrases=kp, confidence=0.75)
