...
class TimezoneConverterTool(StructuredTool):
    ...
    description: str = "Convert a datetime between timezones. Handles DST automatically."
    ...
    def execute(self, inp: TimezoneInput) -> TimezoneOutput:
        try:
            ...
            offset = converted.utcoffset().total_seconds() / 3600 if converted.utcoffset() else None

            human = converted.strftime("%A, %d %B %Y at %I:%M %p %Z") if inp.to_human else None
