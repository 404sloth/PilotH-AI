...
class ActionItemTrackerTool(StructuredTool):
    ...
    def execute(self, inp: ActionTrackerInput) -> ActionTrackerOutput:
        from integrations.data_warehouse.meeting_db import save_action_items, get_person_by_email

        tracked: List[TrackedItem] = []
        db_items = []

        for item in inp.action_items:
            person = get_person_by_email(item.assignee_email) if item.assignee_email else None
            ...
            tracked.append(TrackedItem(
                task_id=task_id,
                description=item.description,
                assignee_name=person["full_name"] if person else item.assignee_email,
                assignee_id=person["id"] if person else None,
                due_date=item.due_date,
                priority=item.priority,
