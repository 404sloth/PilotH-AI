...
class AgendaInput(BaseModel):
    ...
    attendee_roles:   List[str]        = Field(default_factory=list, description="Roles of attendees")
    past_context:     Optional[str]    = Field(None, description="Notes from previous related meetings")
...
class AgendaGeneratorTool(StructuredTool):
    ...
    description: str = "Generate a time-boxed meeting agenda from meeting goals and attendee context."
...
Ensure total duration_mins equals {inp.duration_minutes}. Last item should be 'Next Steps & Action Items'."""
    ...
    def _default_agenda(self, inp: AgendaInput) -> List[AgendaItem]:
        ...
        q, r  = divmod(body, 3)
        return [
            AgendaItem(order=1, topic="Welcome & Objectives",               duration_mins=q + r, description="Set context"),
            AgendaItem(order=2, topic="Status Update",                      duration_mins=q,     description="Current progress"),
            AgendaItem(order=3, topic="Main Discussion: " + inp.goals[:50], duration_mins=q,     description="Core agenda"),
            AgendaItem(order=4, topic="Next Steps & Action Items",          duration_mins=fixed,  description="Assign owners"),
        ]
